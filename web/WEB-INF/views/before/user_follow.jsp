    ...
    <div class="row">
        ...
        <div class="col-md-2">
            <ul class="clearfix">
                ...
                <li style="display:block">
                    <a class="list-group-item active"  title="我的关注" href="user/user_myFollow.action">我的关注</a>
                </li>
                <li style="display:block">
            ...
            <div class="panel panel-default ">
                <div class="container-fluid">
                    <div style="margin: 20px">
                        <form class=" form-inline" action="user/user_myFollow.action" method="GET" id="form-search">
                        ...
                        <div class="panel panel-default">
                            <ul class="list-group">
                                <c:forEach var="user_follow" items="${pageBean.list }">
                                    <a href="javascript:goGet('user/user_personCenter.action', {'uid':'${user_follow.user_id}'})" class="list-group-item">
                                        <h4>${user_follow.user_id}
                                            <c:choose>
                                                <c:when test="${user_follow.user_level == '学生'}">
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${user_follow.user_student.student_desc == ''}">
                                                            <span style="margin-left: 30px;margin-right: 20px;font-size: small;text-align: center">这个人很懒！什么也没留下！</span>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <span style="margin-left: 30px;margin-right: 20px;font-size: small;text-align: center">${user_follow.user_student.student_desc}</span>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </c:when>
                                                <c:otherwise>
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${user_follow.user_teacher.teacher_desc == ''}">
                                                            <span style="margin-left: 30px;margin-right: 20px;font-size: small;text-align: center">这个人很懒！什么也没留下！</span>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <span style="margin-left: 30px;margin-right: 20px;font-size: small;text-align: center">${user_follow.user_teacher.teacher_desc}</span>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </c:otherwise>
                                            </c:choose>
                                            ...
                                            <div class="navbar-right" style="margin-right: 10px">
                                                <button title="取消关注" class="btn btn-danger btn-sm" onclick="delFollow('${user_follow.user_id}');return false;">取消关注</button>
                                            </div>
                                        </h4>
                                    </a>
                                </c:forEach>
...
<script>
    ...
    function delFollow(uid) {
        $.ajax({
            async:true,
            type:'POST',
            url:'user/user_delFollow.action',
            data:{'uid':uid},
            dataType:'json',
            success:function (data) {
                window.location.href(goPost("user/user_myFollow.action",{'code':data.code}));
            },
            error:function () {
                alert("操作失败！");
